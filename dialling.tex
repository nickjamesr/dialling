...
of the determinant. The first step is to place the \(j=0\) column,
\(\vec{c}_{0}\), as the rightmost column. The matrix is then lower triangular,
apart from the upper \(n-1\) terms in \(\vec{c}_{0}\), which are successively
set to zero with the following set of operations:
\begin{equation}
  \vec{c}_{0}^{\left( k \right)} = \vec{c}_{0}^{\left( k-1 \right)} - c_{k}
  \frac{\mat{J}_{k-1,0}^{\left( k-1 \right)}}{J_{k-1,k}}
\end{equation}
where \(k\) runs from \(1\) to \(m-1\) and the \(k^{\text{th}}\) operation
eliminates the \(\left(k-1\right)^{\text{th}}\) element of \(\vec{c}_{0}\) and
removes dependence on \(r_{k}\) from all remaining entries. After this process
is complete, only one non-zero entry remains in \(\vec{c}_{0}\) and its value is
1.
% Anthony - you had a proof that this is the case, but I didn't understand it
%           If you can reproduce it here, that would be great.
We can now compute the determinant by multiplying the diagonal elements of
the matrix:
\begin{equation}
